...
print "Enter input file name for the number of unis(.txt):\n";
$infilename2 = <>;
chomp($infilename2);

$plottitle[1] = "Number of Universities Per Province ";

$xaxis[1] = "Province";
 
$yaxis[1] = "Total Number of Universities";
...
$R->run(q`p2 <- ggplot(data2, aes(x=Province, y=Value)) + geom_bar(stat = "identity", fill="steelblue") + ggtitle(plottitle2) + ylab(ylab2) + xlab(xlab2) + theme(axis.text.x = element_text(angle = 90, hjust = 1))`);
