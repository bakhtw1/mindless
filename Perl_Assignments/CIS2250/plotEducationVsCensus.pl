...
$R->run(qq`xlabel <- "Canadian Population"`);
$R->run(qq`ylabel <- "Number of People"`);
...
#$R->run(qq`counts <- table(data)`);
#$R->run(q`barplot(counts, main="Educated Population and Total Population)`);
$R->run(q`ggplot(data,aes(x=Characteristic, y=Value)) + geom_bar(stat = "identity") + xlab(xlabel) + ylab(ylabel)`);
