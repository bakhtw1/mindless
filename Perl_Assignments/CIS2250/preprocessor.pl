...
my $SLASH = q{/};
my $outfile;
my $outfile_name = "emplymentrates.csv";
my $filename;
my @statistics;
my $percentage;
my $count = 0;
...
foreach my $i (@statistics)
{
    if($csv_COM->parse($i))
    {
        ...
        if( $masterfields[0] =~ m/2011/
            ...
            and $masterfields[3] eq "Total, all education levels" )
        {
	        print FILE $masterfields[0].$COMMA.$masterfields[1].$COMMA.$masterfields[4].$COMMA.$masterfields[5].$COMMA.$masterfields[8]."\n";
        }
    }
